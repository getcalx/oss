...
**Conflict detection before promotion:** Before auto-promotion, `detect_conflicts` checks for opposing verbs on overlapping subjects across existing rules. If a new rule would contradict an active rule (e.g., "always use relative imports" vs "never use relative imports"), promotion is blocked and the conflict is surfaced for human resolution. This prevents the rule set from containing contradictions that would confuse agents.
