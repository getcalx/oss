...
def test_check_recurrence_finds_match_same_domain(tmp_path: Path):
    ...
    c1 = create_correction(calx_dir, domain="api", description="don't mock the database in tests")
    c2 = create_correction(calx_dir, domain="api", description="stop mocking database in tests")
...
def test_check_recurrence_appends_event(tmp_path: Path):
    ...
    c1 = create_correction(calx_dir, domain="api", description="don't mock database connections")
    c2 = create_correction(calx_dir, domain="api", description="stop mocking database connections")
...
def test_check_recurrence_ignores_different_domain(tmp_path: Path):
    ...
    create_correction(calx_dir, domain="api", description="don't mock database connections")
    c2 = create_correction(
        calx_dir, domain="tests", description="don't mock database connections"
    )
